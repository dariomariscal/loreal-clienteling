...
/**
 * Two-tier nav for the Area Manager.
 *
 * Tier 1 (Operación) — daily-pulse style screens that mirror the advisor's
 * "Hoy" entry point but at the zone level.
 * Tier 2 (Análisis) — denser, drill-down screens (heatmap, inventory matrix,
 * swimlane events). These exist as separate destinations because they
 * deliberately break the chasis (full-bleed / Tier-3 density).
 */
const OPERATION_NAV: NavItem[] = [
  { href: "/area-manager/today", label: "Mi zona hoy", icon: RoutineMorningGlyph },
...
const ANALYSIS_NAV: NavItem[] = [
  { href: "/area-manager/heatmap", label: "Mapa de clientas", icon: MapPinGlyph },
  { href: "/area-manager/inventory", label: "Inventario", icon: PackageGlyph },
  { href: "/area-manager/events", label: "Eventos", icon: CalendarPlusGlyph },
];
...
export function AreaManagerNav({ user, onNavigate, collapsed = false }: Props) {
              ...
              <Tooltip.Trigger
                render={
                  <Link
                    href={ACCOUNT_HREF}
                    onClick={onNavigate}
                    aria-label={`${user.fullName || "Gerente de zona"} — Mi cuenta`}
                    className={cn(
                  ...
                  >
                    <CustomerAvatar
                      firstName={user.fullName || "Gerente de zona"}
