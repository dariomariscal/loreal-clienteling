...
import { NationalNav } from "./national-nav";
 ...
 */
export function NationalSidebar({ user, forceRail = false }: Props) {
  if (forceRail) {
    return (
      <nav
        aria-label="National navigation"
        className="hidden md:flex h-dvh w-[72px] flex-col border-r border-[color:var(--ba-sidebar-border)]"
      >
        <NationalNav user={user} collapsed />
      </nav>
    );
  }
