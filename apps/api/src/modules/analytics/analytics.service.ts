...
export class AnalyticsService {
   ...
   *
   * Industry naming: "Appointment overview" is the Tulip / BSPK term for the
   * BA's metrics surface. KPIs match Endear's "Clienteling KPIs" doc.
   */
  async getAppointmentOverview(user: SessionUser, range?: DateRange) {
    const isAdmin = user.role === UserRole.ADMIN;
    const isBA = user.role === UserRole.BEAUTY_ADVISOR;
    const storeIds = await this.scopeService.getAccessibleStoreIds(user);
    const { from, to } = this.getDefaultDateRange(range);

    // ── Scope filter for appointments ────────────────────────────
    ...
    // Same scope expressed for orders (attribution layer).
    ...
    // ── Outcome / reason breakdowns with pct ─────────────────────
    ...
    // ── Trend buckets need revenue join per week ─────────────────
